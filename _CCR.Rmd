...
Carozzi, F., Cipullo, Da. & Repetto, L. (2022). Political Fragmentation and Government Stability: Evidence from Local Governments in Spain. _American Economic Journal: Applied Economics_, 14 (2): 23-50. <a href="https://www.aeaweb.org/articles?id=10.1257/app.20200128" target="_blank">https://www.aeaweb.org/articles?id=10.1257/app.20200128</a>
...
## Link in Github repo
download_link(
  link = "https://git-link.vercel.app/api/download?url=https%3A%2F%2Fgithub.com%2FSorbonneDevEcon%2FM2-Econometrics-Book%2Fblob%2Fmain%2Fresources%2FAA_ALC_AT%2FDoFile_CCR.do&filename=DoFile_CCR.do",
...
## Link in Github repo
download_link(
  link = "https://git-link.vercel.app/api/download?url=https%3A%2F%2Fgithub.com%2FSorbonneDevEcon%2FM2-Econometrics-Book%2Fblob%2Fmain%2Fresources%2FAA_ALC_AT%2FCodebook_CCR.pdf&filename=Codebook_CCR.pdf",
...
download_link(
  link = "https://www.openicpsr.org/openicpsr/project/125341/version/V1/view?path=/openicpsr/125341/fcr:versions/V1/PFGS_Replication_package_AEJ/Data/Clean&type=folder",
  button_label = "Download Original Data (from Open ICPSR: click on Data_final.dta and Data_final_allparties.dta)",
  button_type = "default",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
<a href="https://www.openicpsr.org/openicpsr/project/125341/version/V1/view" target="_blank">[Link to the full original replication package paper on OSF]</a>
...
save "$dt\Dataset_CCR.dta", replace
```
...
clear all
set more off
global dt "C:\Users\Dell\Desktop\laurine_stata_files\CCR"
```
...
esttab panelA, cells((mean(fmt(%18.2fc)) sd(fmt(2)) min(fmt(1)) max(fmt(0)))) nomtitles ///
  ...
  addnotes(Notes: "Panel A provides descriptives at the municipal level for all municipalities that appear at least once in our sample.")
```
...
esttab panelB, cells((mean(fmt(%18.2fc)) sd(fmt(2)) min(fmt(1)) max(fmt(0)) )) nomtitles ///
  ...
  obslast varwidth(35) label ///
  addnotes(Notes: "Panel B provides descriptives on electoral outcomes at the municipality-council level.")
```
...
esttab panelC1, cells((mean(fmt(%18.2fc)) sd(fmt(2)) min(fmt(1)) max(fmt(0)) )) ///
  ...
  addnotes(Notes: "Panel C splits this sample into councils that approved at least one vote of no confidence during the term (C2), and those that did not(C1)")
```
...
local CCT_bw_l = e(h_mserd) // stores the value of the estimated bandwidth
```
